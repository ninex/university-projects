...
//  static unsigned char tga24[14] = {0,0,2,0,0,0,0, 0,0,0,0,0,24,32};
  ...
  for (i=0; i<height; i++)
  {
    ...
    for (j=0; j<horizontal_size; j++)
    {
      ...
      if (current  != NULL){
	r1=0;g1=0;b1=0;
	temp = (long)current->r + i*current->w+j;
	memcpy(&r1,(void*)temp,1);
	
	temp = (long)current->g + i*current->w+j;
	memcpy(&g1,(void*)temp,1);
	
	temp = (long)current->b + i*current->w+j;
	memcpy(&b1,(void*)temp,1);
      
      
      if (j> horizontal_size*0.3 && j < horizontal_size*0.6 && i > height *0.5)
      {
      ...
      /*
			...
			if (j> horizontal_size*0.3 && j < horizontal_size*0.6 && i > height *0.5)
				if (((rm - 1) < r) && ((rm + 1) > r) && ((gm - 1) < g) && ((gm + 1) > g) && ((bm - 1) < b) && ((bm + 1) > b)){
						points++;
				}
			//}
		...
		if (!(current = (ptrFrame*)malloc(sizeof(struct ptrFrame)*2))){
...
/*
	framers *tempF;
 	if ( (!sd) && (!mean) && framecount > 1 ){
		
		//printf("%d dit is:%.10f\n",framecount,(double)(points*100) / (height*horizontal_size));
		//printf("%d dit is:%.10f\n",framecount,(double)(points) );
		if (points < 3){
		//printf("it is:%.10f\n",(double)(points*100) / (height*horizontal_size));
			if (spl < 2){
				spl = 0;
			}else{
				if ((framecount - last) > 25){
					printf("Split frame %d\n",framecount);
					spl = 0;
					
					tempF = (framers*)malloc(sizeof(struct framers));
					tempF->begin = last;
					tempF->end = framecount;
					tempF->next = NULL;
...
*/
//printf("Frame %d Points %f\n",framecount,points/(height*horizontal_size));		
		framers *tempF;
		if ((double)points*100/(height*horizontal_size) < 13){
			if (spl < 3){
				spl++;
			}else{
				if ((framecount - last) > 25){
...
// 		if ((double)points*100/(height*horizontal_size) < 1.4){
// 			if (spl < 3){
// 				spl++;
// 			}else{
// 				if ((framecount - last) > 25){
