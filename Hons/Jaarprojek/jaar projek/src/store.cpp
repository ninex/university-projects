...
//  static unsigned char tga24[14] = {0,0,2,0,0,0,0, 0,0,0,0,0,24,32};
  ...
  else
  {
	...
	optr = obfr;
	framecount++;
//if ((framecount >= target) && (framecount <= target+5)){

	QImage picture;
	
	
	picture.create(horizontal_size,height,32,0,QImage::IgnoreEndian);
	
	
	printf("Processing frame %d\n",framecount);
		if (first == NULL){
			//printf("Allocating first frame\n");
			if (!(first = (ptrFrame*)malloc(sizeof(struct ptrFrame)*2))){
		...
		//	printf("Hier0\n");
			ptrFrame *temp;
			if (!(temp = (ptrFrame*)malloc(sizeof(struct ptrFrame)))){
				printf("Malloc failed\n");
			}else{
		//	printf("Hier1\n");
				current->next = temp;
		...
		current->h = height;
		//else{
		//current->picture = picture.copy(0,0,horizontal_size,height);
	
	
	int temp;
//picture.scaleWidth(horizontal_size);
//picture.scaleHeight(height);
  ...
  for (i=0; i<height; i++)
  {
    ...
    for (j=0; j<horizontal_size; j++)
    {
      ...
      temp = (int)current->r + j*horizontal_size+i;
      memset((void*)temp,r,1);
      temp = (int)current->g + j*horizontal_size+i;
      memset((void*)temp,g,1);
      temp = (int)current->b + j*horizontal_size+i;
