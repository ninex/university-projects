...
unsigned int get_align(mem_block* first,unsigned int align_to)
{//printk("oond\n");
  ...
  if (align_to == 0x10000)
  {
    ...
    {    
      ...
      }else
      {
        ...
        return i386_align((unsigned int)first+size,0x10000);
      }
    }
  ...
  {//printk("oond2\n");
    ...
    }else
    {
      ...
      return i386_align((unsigned int)first,align_to);
    }
  }
}
...
void *allocate(unsigned int size,unsigned int align_to,int tag)
{
  ...
  }else
  {
    ...
    if ((align_to != 0) && ((unsigned int)first != get_align(first,align_to)))
    {
...
void mem_initialize() {
  ...
  }else
  {
    ...
    #ifdef DEBUG_MEM
      printk("Kernel page not loaded\n");
    #endif
  }  
  #ifdef DEBUG_MEM
    printk("Initialized memory\n");
  #endif
}
