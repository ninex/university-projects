...
void ERR_HANDLER(int error){
	...
	case 0 : printf("Error : Undefined\n");
		exit(EXIT_FAILURE);
		break;
	case 1 : printf("Error : line [%d] : no file open or file does not exist\n", line_num);
		exit(EXIT_FAILURE);
		break;
	case 2 : printf("Error: line [%d] : maximum identifier length exceeded\n", line_num);
		exit(EXIT_FAILURE);
		break;
	case 3 : printf("Error: line [%d] : comment not closed\n", line_num);
		exit(EXIT_FAILURE);
		break;
	case 4 : printf("Error: line [%d] : integer value out of range\n", line_num);
		exit(EXIT_FAILURE);
		break;
	default: printf("Error Default: line [%d] Undefined\n",line_num);
		exit(EXIT_FAILURE);
		break;
	}
}
...
void readChar(){
	if (fptr!= NULL){
		fscanf(fptr,"%c",&sym);
		if (sym == 10) {
			//line_num++; took this out as the linenumber was wrong by double. need to fix this.
		}
		if (feof(fptr)!= 0){
...
token_str* getToken(){
	...
	while ((sym == 9) || (sym == 11) || (sym == 12) || (sym == 13))
	{
		readChar();
	}	
