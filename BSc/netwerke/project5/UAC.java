...
public class UAC extends Thread implements Runnable
{
	...
	private boolean running,docall,onRedirect = false;
	private int port;
	private String who;
	...
	public void run()
	{
		...
		while(running)
		{
			...
			{
				makeCall();
				//Point pa = (MouseInfo.getPointerInfo()).getLocation();
	...
	public void makeCall()
	{
		...
		try
		{
			...
			if ((header[1].compareTo("100")) == 0)
			{
				...
				//if 180, proceed
				if (((header[1].compareTo("180")) == 0)  || ((header[1].compareTo("302")) == 0))
				{
					if ((header[1].compareTo("302")) == 0)
					{
						onRedirect = true;
						VoIp.addMsg("[UAC] Redirect received from proxy");
						/*String stuff2[] = SIP.stripResponse(in_msg);
						String stuff3[] = stuff2[2].split("\n");
						stuff2 = stuff3[0].split("address:");
						
						remoteAddr = stuff2[1];
						System.out.println(remoteAddr);
						stuff2 = stuff3[1].split("port:");
						System.out.println(stuff2);
						remotePort = (new Integer(stuff2[1].trim())).intValue();
						*/
						...
						to = who+" <sip: "+who+"@"+remoteAddr+">";
						from = me+" <sip:"+me+"@"+addr+">";
						msgheader = SIP.Header(to,from,"proxy: "+proxyA);
						VoIp.addMsg("[UAC] Sending INVITE to "+remoteAddr+":"+remotePort);
						msg = SIP.createRequest("INVITE",remoteAddr,remotePort,msgheader,"");
						outgoing.send(msg);
						...
						if ((header[1].compareTo("100")) == 0)
						{
							VoIp.addMsg("[UAC] Received TRYING = "+header[1]+" from "+(in_msg.getAddress()).getCanonicalHostName()+", protocol"+header[0]);
							...
							{
								VoIp.addMsg("[UAC]Received Ringing = "+header[1]+" from "+(in_msg.getAddress()).getCanonicalHostName()+", protocol"+header[0]);
							}
						}
					}else
					{
					...
					//Point p = VoIp.getLocation();
					
					/*for (int i=0;i<100;i++)
					{
						VoIp.setLocation(p.x+i%4,p.y+i%4);
					}*/
					//VoIp.setLocation(p.x,p.y);
					...
					while (wait)
					{
						...
						if ((header[1].compareTo("200")) == 0)
						{
							...
							{
								VoIp.addMsg("[UAC]Sending ACK to "+remoteAddr+" port:"+remotePort);
							}
	...
	public void voiceConnect()
	{
		if (VoIp.audioEnabled())
		{
			...
			try
			{
				...
				speak = new speaker(talk);
				speak.start();
				VoIp.addMsg("[UAC] You have speaker capabilities");
			}catch (Exception e){e.printStackTrace();}
		}
	}
}