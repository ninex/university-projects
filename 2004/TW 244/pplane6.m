...
elseif strcmp(action,'proceed')
   ...
   if (sud.flag == 1)
	 ...
	 if isempty(pv) 
	   ...
	   xxderivstr = pplane6('paraeval',pn,pv,xderivstr);
	   yyderivstr = pplane6('paraeval',pn,pv,yderivstr);
	...
	otherwise
	 ...
	 set(gcf,'defaultuicontrolfontsize',fs*0.9);
       ...
       mequit = uimenu(fmenu,'label','Quit PPLANE6',...
		       'call','pplane6(''quit'')','separator','off');
	...
	ode113ch = 'off';
	 ...
	 ode23tbch = 'off';
       ...
       set(ppdisp,'WindowButtonMotionFcn','pplane6(''cdisp'')');
...
elseif strcmp(action,'kcompute')
   ...
   for kk = 1:6;
	 ...
	 xstr = pplane6('paraeval',pn,pv,xstr);
	 ystr = pplane6('paraeval',pn,pv,ystr);
       end
     end
   end
   xvalue = str2num(xstr);
...
elseif strcmp(action,'settings')
   ...
   setfig = figure('name','PPLANE6 Settings',...
		   'numb','off',...
		   'tag','PPLANE6','vis','off');
...
elseif strcmp(action,'plxybdf')
   ...
   dispa = gca;
   dispf = gcf;
   set(dispf,'windowbuttonmotionfcn','pplane6(''plxybmf'')',...
...
elseif strcmp(action,'zoomsqu')
   ...
   set(disph,'user',dud);
   set(disph,'WindowButtonDownFcn','pplane6(''down'')',...
      'WindowButtonMotionFcn','pplane6(''cdisp'')',...
      'WindowButtonUpFcn','');
   pplane6('dirfield',disph);
   ppset = findobj('name','PPLANE6 Setup');
   if isempty(ppset)
...
elseif strcmp(action,'levcomp')
  ...
  for kk = 1:4;
	...
	pv(find(abs(pv)==32))=[];
	lfcn = pplane6('paraeval',pn,pv,lfcn);
      end
    end
  end
  l = length(lfcn);
...
% PPOUT  is the output function for PPLANE6.

%  Copywright (c)  John C. Polking, Rice University
%  Last Modified: January 21, 2001

output = 0;
ppdisp = findobj(get(0,'child'),'flat','name','PPLANE6 Display'); %gca;   % 
dud = get(ppdisp,'user');
ppdispa = dud.axes;
dfcn = dud.function;
ud = get(ppdispa,'user');
